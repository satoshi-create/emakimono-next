# GitHub Actions 同期実行・運用マニュアル（アーカイブ）

> **非推奨 — 参照用のみ**  
> Supabase 時代の旧手順です。**現行パイプラインは [scroll-pipeline.md](../operations/scroll-pipeline.md) を正とします。**  
> 本書には push トリガーでの Actions 起動など、**現行と矛盾する記述**が含まれます。
...
- 現行パイプライン: [`scroll-pipeline.md`](../operations/scroll-pipeline.md)
