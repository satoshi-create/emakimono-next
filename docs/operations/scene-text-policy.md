...
| `gendaibun` | 推奨 | 現代文。詞書がある巻 = 詞書の現代語訳。詞書がない巻 = 場面の平易な描写。**1〜3 文** |
| `kobun` | 任意 | 原文（古文。あれば） |
| `desc` / `descen` | 推奨 | 解説文（深堀り層）。背景・絵画表現・文化的文脈を **3〜6 文** |
...
```
層1: オーバーレイ本文（gendaibun）
  シーン表示中に即座に見える現代文。短く、漢語を避け、情景を具体的に描写。
  → 絵巻に「興味を持った」段階

層2: 各段の解説（desc / descen）
  アコーディオン / 解説モーダル。背景・技法・文化文脈を深堀り。
  → 絵巻を「深掘りしたくなった」段階
```
