...
const useEmakiAutoPlay = ({
  ...
  const stopPlayMode = () => {
    ...
    if (playbackStrip) {
      ...
      playbackStrip.syncScrollLeftFromOffset();
  ...
  const startPlayMode = () => {
    ...
    if (playbackStrip && !playbackStrip.initFromScrollLeft()) return;
    ...
    const playScroll = (ts) => {
      ...
      if (playbackStrip) {
        ...
        const { reachedEnd } = playbackStrip.tick(ts);
